...
def compute_total_energy_LF(p: Pendulum):
    ...
    V = 1/2 * p.m * (L**2 * state[1]**2 + Ldot**2) - p.m * p.g * L * np.cos(state[0]) + p.m * p.g * p.l0
...
def compute_total_energy_LF_derivative_f(p: Pendulum):
    phi = np.hstack((p.wave_phi[-1], p.wave_dphi[-1]))
    f = p.variable_length_eom(phi)

    dVdphi = 1/2 * p.m * (p.l0 * phi[1]**2 * (2 * p.delta**2 * phi[0] * phi[1]**2 + 2*p.delta*phi[1]) +
                          p.l0**2 * p.delta**2*(2*phi[0]*f[1]**2+2*phi[1]**2*f[1])) + \
             p.m*p.g*p.l0*(-p.delta*phi[1]*np.cos(phi[0])+p.delta*phi[0]*phi[1]*np.sin(phi[0]))

    dVddphi = 1/2 * p.m * (p.l0 * 2 * phi[1] * (1 + p.delta**2*phi[0]**2*phi[1]**2) +
                           p.l0 * phi[1]**2*(2*p.delta**2*phi[0]**2*phi[1]+2*p.delta*phi[0]) +
                           p.l0*p.delta**2*(4*phi[0]**3+4*phi[0]*phi[1]*f[1]))-p.m*p.g*p.l0*p.delta*phi[0]*np.cos(phi[0])
...
# plt.show()
plt.savefig('LF_TotalEnergy_Vdot.png', format='png', dpi=300)
plt.close(fig)
