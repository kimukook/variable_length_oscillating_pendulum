...
def compute_total_energy_LF(p: Pendulum):
    ...
    V = 1/2 * p.m * (L * state[1]**2 + Ldot**2 / L) + p.m * p.g * (p.l0 / L - np.cos(state[0]))
...
def compute_total_energy_LF_derivative(p: Pendulum):
    ...
    dVdphi = 1/2 * p.m * (p.l0*p.delta*phi[1]**3 + (p.l0**3*p.delta**2*(2*phi[0]*f[1]**2+2*phi[1]**2*f[1]) *
                                                     (1+p.delta*phi[0]*phi[1])-p.l0**3*p.delta**3*phi[1]*(phi[0]*f[1]+phi[1]**2)**2)/(p.l0**2*(1+p.delta*phi[0]*phi[1])**2)) \
             + p.m*p.g*((-p.delta*phi[1])/(1+p.delta*phi[0]*phi[1])**2+np.sin(phi[0]))

    dVddphi = 1/2*p.m*(2*phi[1]*p.l0+3*p.delta*p.l0*phi[0]*phi[1]**2+(p.l0**3*p.delta**2*(4*phi[1]**3+4*phi[0]*phi[1]*f[1])*(1+p.delta*phi[0]*phi[1])-p.delta**3*p.l0**3*phi[0]*(phi[0]*f[1]+phi[1]**2)**2)/(p.l0**2*(1+p.delta*phi[0]*phi[1])**2))+p.m*p.g*((-p.delta*phi[0])/(1+p.delta*phi[0]*phi[1])**2)
...
io.savemat('LF_total_energy_variation_division.mat', LF)
...
# plt.show()
plt.savefig('LF_TotalEnergy_Vdot_division.png', format='png', dpi=300)
plt.close(fig)
